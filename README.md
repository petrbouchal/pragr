...
only place where the full current Spatial plan lives
(<http://mpp.iprpraha.cz/arcgis/rest/services/PUP>).

For some of the services, shortcut notation is implemented for use in
the tile/basemap functions. See `prg_endpoints`. **Not all endpoint
metadata in `prg_endoints` has been updated to reflect the new URLs.**

## Note on geodata about Prague

There will be a [vignette](articles/data-sources.html) or
